...
## 5.57 抽象类和接口的区别，分别在什么场景下使用

- 定义：
  - 抽象类：含有纯虚函数的类就是抽象类，而且它还能包含非纯虚函数和成员变量，且不能被实例化，只能作为基类被继承，单继承
  - 接口：全部由纯虚函数构成，没有成员变量，也不存在函数的实现，没有构造函数，支持多重继承

- 继承规则：
  - 抽象类：子类必须对所有纯虚函数进行实现，但可以选择不实现普通函数
  - 接口：子类要实现接口里的全部方法

- 适用场景：
  - 抽象类：当多个类有共同的状态（成员变量）或者行为（非纯虚函数）时，比如不同种类的动物都有进食的行为。表示“is-a”关系，作为类族共同的基类，提供模板方法模式。
  - 接口：当需要定义一种规范或者能力，并且有多个不相关的类要实现它时，比如不同类型的飞行器都需要实现飞行相关的接口。表示“can-do”或“has-a”关系，定义行为准则/标准，实现多态，弥补单继承机制的不足。
